...
const CreateAssessment = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Submitting data:', submissionData); // Add this line for debugging

      const token = localStorage.getItem('accessToken');
      const response = await axios.post(
        'https://lms1-1-p88i.onrender.com/api/assessments/',
        submissionData,
        {
          headers: {
      ...
      if (error.response?.data) {
        ...
        setErrors({ 
          form: 'Failed to create assessment. Please try again.' 
        });
      }
    } finally {
      setLoading(false);
